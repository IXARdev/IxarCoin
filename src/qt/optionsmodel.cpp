...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeIxarcoinAmount"))
        settings.setValue("nAnonymizeIxarcoinAmount", 1000);
    nAnonymizeIxarcoinAmount = settings.value("nAnonymizeIxarcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeIxarcoinAmount"))
        SoftSetArg("-anonymizeixarcoinamount", settings.value("nAnonymizeIxarcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeIxarcoinAmount:
            return QVariant(nAnonymizeIxarcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeIxarcoinAmount:
            nAnonymizeIxarcoinAmount = value.toInt();
            settings.setValue("nAnonymizeIxarcoinAmount", nAnonymizeIxarcoinAmount);
            emit AnonymizeIxarcoinAmountChanged(nAnonymizeIxarcoinAmount);
            break;
        default:
            break;
        }
