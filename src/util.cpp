...
int nAnonymizeIxarcoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ixarcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ixarcoin
    // Mac: ~/Library/Application Support/Ixarcoin
    // Unix: ~/.ixarcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ixarcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ixarcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ixarcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong IXARCOIN will not work properly.");
